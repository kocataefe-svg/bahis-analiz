...
| Groq API | AI analiz metni üretimi — Takım Analizcisi + Yorumcu + özet (arka plan, `openai/gpt-oss-20b`) | Ücretsiz | Free tier, model başına RPM/TPM limiti (bkz. console.groq.com/docs/rate-limits) |
| Gemini API (Google) | (1) İsteğe bağlı sakatlık/form/H2H araştırması (Google Search grounding, "Araştır" butonu) — (2) AI analizin Bahis Analizcisi + Sürpriz Yorumcu kısmı (arka plan, `gemini-3.5-flash-lite`, grounding YOK, ayrı kota havuzu) | Ücretsiz | Search grounding: 5.000 istek/ay (paylaşımlı); normal üretim: 1500 istek/gün, 30 istek/dk |
...
- **Taraf bahsi (h2h) disindaki pazarlar otomatik toplu çekimde yok, maç bazında tamamlanıyor:** The Odds API bu piyasaları toplu (`/sports/{key}/odds`) endpoint'inde sunmuyor — sadece maç başına ayrı bir endpoint'te (`/sports/{key}/events/{id}/odds`) mevcut, ve her piyasa×bölge ayrı kredi harcıyor. Bunun yerine, bir maçın eksik piyasaları iki durumda tek seferlik çekilip `odds_snapshots`'a kalıcı olarak yazılır (`src/lib/odds-enrichment.ts`): (1) bir kullanıcı o maçın detay sayfasını ilk kez açtığında, (2) `sync/analysis` o maçı analiz etmeden önce. Her iki durum da önce cache'e bakar, sadece eksikse çeker. 2026-09-15'te kapsam genişledi — `totals` (2.5 çizgisi), `btts`, `h2h_h1` (ilk yarı taraf bahsi), `totals_h1` (ilk yarı 1.5 çizgisi), `btts_h1` (ilk yarı KG), `spreads` (handikap, sadece ±1/±2 çizgileri tutulur), `player_goal_scorer_anytime` (gol atacak oyuncu — 30+ oyuncudan sadece en düşük 2 oranlı "favori" ile orta-üst siradan 1 "sürpriz" seçilip saklanır, tümü değil). AI analiz promptunda sadece h2h/totals/btts zorunlu (her biri icin ayri "Tahminim:" gerektirir); digerleri opsiyonel baglam olarak sunulur, Bahis Analizcisi ve Surpriz Yorumcu firsat buldukca kullanir.
- **Kota hesabı ve bütçe riski (2026-09-15 güncellemesi, pazar sayisi artisiyla yeniden hesaplandi):** The Odds API krediyi **istek başına değil, piyasa×bölge başına** faturalandırıyor. 11 lig × 1 piyasa (`h2h`) × 1 bölge (`eu`) × günde 1 senkron × 30 gün ≈ **330 kredi/ay** (toplu 1X2 senkronu, degismedi). Maç-bazlı tamamlama artık 7 piyasa aynı anda istendiği için (7 kredi/maç, ilk cekimde) çok daha pahalı — `sync/analysis`'in günlük ~15 maçı proaktif tamamlaması teorik olarak +100 kredi/gün'e kadar çıkabilir (kalıcı cache sayesinde pratikte çok daha az — sadece pencereye YENİ giren maçlar gerçek API çağrısı tetikler). **Toplam aylık kullanım 500 kredi/ay limitini kolayca aşabilir.** Bu kasıtlı bir tercih: aşılırsa senkron sessizce başarısız olmaz, sadece o ayki eksik piyasalar için "veri yok" yazmaya devam eder (uydurma riski yok, sadece eksik kalır, Odds API kotası ay başında sıfırlanınca kendiliğinden düzelir). Gerçek kullanımda sorun olursa `MAX_MATCHES_PER_RUN` düşürülmeli veya opsiyonel pazarlardan biri (en olası aday: `player_goal_scorer_anytime`, en dar kapsamli ve en az kritik) kaldırılmalı.
...
- **Gemini model güncellemesi (Plan 3 öncesi düzeltme, tarihsel):** İlk tasarımda seçilen `gemini-2.5-flash-lite`'ın emekliye ayrılacağı doğrulanmıştı, `gemini-3.5-flash-lite`'a geçildi. Bu model hâlâ **sadece Araştır (Google Search grounding)** için kullanılıyor — `ai.models.generateContent({ model, contents, config: { tools: [{ googleSearch: {} }] } })`.
- **2026-09-15: arka plan analizi Gemini'den Groq'a taşındı.** Sebep: kullanıcının Gemini API key'i paylaşımlı bir Google Cloud projesine bağlıydı, o projede başka bir uygulama için billing hesabı açılmıştı — proje bazlı çalıştığı için TÜM Gemini çağrıları (bu uygulamanınki dahil) otomatik ücretli tier'a düşüp gerçek harcama oluşturdu. Billing'siz yeni bir proje/key denendi ama Vercel'in sunucusuz ortamından tutarsız `401`/timeout hataları alındı (yerelden ayni key sorunsuz çalıştı) — kesin sebep netleşmedi. Kalıcı çözüm olarak arka plan analizi Groq'a taşındı.
- **2026-09-15: 4 persona iki saglayiciya bölündü (tek saglayicinin kota/rate-limit riskini azaltmak icin).** Groq (`src/lib/groq.ts`, `openai/gpt-oss-20b`) Takım Analizcisi + Yorumcu + özeti üretir - bu "çekirdek" kısım, başarısız olursa analiz tamamen iptal edilir. Gemini (`src/lib/gemini-analysis.ts`, `gemini-3.5-flash-lite`, grounding kullanmıyor - Araştır'ın kotasını paylaşmaz) Bahis Analizcisi + Sürpriz Yorumcu'yu üretir - bu "ek" kısım, başarısız olursa analiz yine de kaydedilir, sadece o iki alan yer tutucu metinle doldurulur. İkisi paralel (`Promise.all`) çağrılır - `src/lib/analysis-orchestrator.ts`. Groq TPM sorunu yaşandığı icin bu bölünme dogrudan fayda saglar (tek cagrida 4 persona yerine 2+2, toplam Groq token yükü yarıya iner). İleride Groq/Gemini de limitlere takılırsa, her personayı ayrı bir key'e (ör. 4 ayrı Gemini projesi) bölmek bir sonraki adım olarak planlandı - kullanıcı bunu acikca istedi, bkz. [[project-bahis-analiz-status]] memory.
...
2. Kullanıcı bir maça tıklar → detay sayfası açılır:
   ...
   - 4 persona analiz: **Takım Analizcisi** (form/sakatlık/önemli an yorumu — örn. "play-off için 3 puana mecburlar"; Araştır verisi varsa mutlaka kullanılır), **Bahis Analizcisi** (istatistik + oran okuma, value değerlendirmesi), **Yorumcu** (genel maç yorumu + mevcut her pazar için net "Tahminim: ..." tahmini, opsiyonel kombine öneri), **Sürpriz Yorumcu** (2026-09-15 eklendi — favoriyi tekrarlamak yerine mevcut pazarları birlikte değerlendirip somut bir "sürpriz kombinasyon" önerir, örn. "MS 2 + KG Var"; Türkiye sitelerinden spesifik oran uydurması promptla yasaklanmıştır, sadece elimizdeki uluslararası referans oranlardan yaklaşık hesap yapar)
