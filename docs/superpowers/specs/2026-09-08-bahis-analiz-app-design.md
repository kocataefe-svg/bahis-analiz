...
- **KG Var/Yok (BTTS) ve 2.5 Alt/Üst (totals) otomatik toplu çekimde yok, maç bazında isteğe bağlı:** The Odds API bu piyasaları toplu (`/sports/{key}/odds`) endpoint'inde sunmuyor — sadece maç başına ayrı bir endpoint'te (`/sports/{key}/events/{id}/odds`) mevcut, ve her piyasa×bölge ayrı kredi harcıyor. Tüm liglere toplu eklemek (14→şimdi 11 lig × 3 piyasa) bütçeyi 2-3x aşardı. Bunun yerine: bir kullanıcı bir maçın detay sayfasını **ilk kez açtığında**, o maça özel eksik piyasalar (`totals` 2.5 çizgisi + `btts`) tek seferlik çekilip `odds_snapshots`'a kalıcı olarak yazılır (`src/lib/odds-enrichment.ts`) — sonraki ziyaretler ve günlük AI analizi bu önbelleklenmiş veriyi kullanır, tekrar API çağrısı yapmaz. Maliyet kullanım bazlı (görüntülenen maç sayısı × 2 kredi), toplu senkrona dahil değil.
- **Kota hesabı (1X2 toplu senkron):** The Odds API krediyi **istek başına değil, piyasa×bölge başına** faturalandırıyor. 11 lig (TFF 1. Lig + Eliteserien/Allsvenskan/Super League kapsam dışı, Milli Maçlar dahil) × 1 piyasa (`h2h`) × 1 bölge (`eu`) × günde 1 senkron × 30 gün ≈ **330 kredi/ay** — 500 kredi/ay kotasının içinde, yukarıdaki maç-bazlı `totals`/`btts` çekimlerine yer bırakıyor. Aynı çağrı hem fikstürü hem oranı getirdiği için (Odds API event objesi `id`/`home_team`/`away_team`/`commence_time`'ı bookmaker verisiyle birlikte döndürüyor) fikstür senkronu ek kota **tüketmiyor**.
