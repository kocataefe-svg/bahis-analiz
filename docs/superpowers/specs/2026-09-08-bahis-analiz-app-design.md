...
| Groq API | AI analiz metni üretimi (arka plan, `openai/gpt-oss-20b`) | Ücretsiz | Free tier, model başına RPM/RPD limiti (bkz. console.groq.com/docs/rate-limits) |
| Gemini API (Google) | Sadece isteğe bağlı sakatlık/form/H2H araştırması (Google Search grounding, kullanıcı "Araştır" butonuyla tetikler) | Ücretsiz | Search grounding: 5.000 istek/ay (Gemini 3.x, paylaşımlı) |
...
- **2026-09-15: arka plan analizi Gemini'den Groq'a taşındı.** Sebep: kullanıcının Gemini API key'i paylaşımlı bir Google Cloud projesine bağlıydı, o projede başka bir uygulama için billing hesabı açılmıştı — proje bazlı çalıştığı için TÜM Gemini çağrıları (bu uygulamanınki dahil) otomatik ücretli tier'a düşüp gerçek harcama oluşturdu. Billing'siz yeni bir proje/key denendi ama Vercel'in sunucusuz ortamından tutarsız `401 invalid_token` hataları ve timeout'lar alındı (yerelden aynı key sorunsuz çalıştı) — kesin sebep netleşmedi, muhtemelen ağ/origin kaynaklı bir kısıtlama. Kalıcı çözüm olarak arka plan analizi (`src/lib/groq.ts`, model `openai/gpt-oss-20b`, OpenAI-uyumlu `/openai/v1/chat/completions` endpoint'i, `response_format: {type:"json_object"}`) gerçekten kredi kartsız ücretsiz olan Groq'a taşındı. Gemini artık sadece nadiren tetiklenen Araştır özelliği için kullanılıyor (billing riski büyük ölçüde azaldı). `GEMINI_API_KEY` sorunu ileride ayrıca çözülebilir, acil değil.
