...
- **KG Var/Yok (BTTS) ve 2.5 Alt/Üst (totals) otomatik toplu çekimde yok, maç bazında tamamlanıyor:** The Odds API bu piyasaları toplu (`/sports/{key}/odds`) endpoint'inde sunmuyor — sadece maç başına ayrı bir endpoint'te (`/sports/{key}/events/{id}/odds`) mevcut, ve her piyasa×bölge ayrı kredi harcıyor. Tüm liglere toplu eklemek (14→şimdi 11 lig × 3 piyasa) bütçeyi 2-3x aşardı. Bunun yerine, bir maçın eksik piyasaları (`totals` 2.5 çizgisi + `btts`) iki durumda tek seferlik çekilip `odds_snapshots`'a kalıcı olarak yazılır (`src/lib/odds-enrichment.ts`): (1) bir kullanıcı o maçın detay sayfasını ilk kez açtığında, (2) `sync/analysis` o maçı analiz etmeden önce (2026-09-15'te eklendi — yorumcuların "veri yok" demek zorunda kalmaması için). Her iki durum da önce cache'e bakar, sadece eksikse çeker.
- **Kota hesabı ve bütçe riski (2026-09-15 güncellemesi):** The Odds API krediyi **istek başına değil, piyasa×bölge başına** faturalandırıyor. 11 lig × 1 piyasa (`h2h`) × 1 bölge (`eu`) × günde 1 senkron × 30 gün ≈ **330 kredi/ay** (toplu 1X2 senkronu). `sync/analysis`'in artık günlük ~15 maç için proaktif olarak KG/2.5 tamamlaması (kalıcı cache sayesinde çoğu gün sadece pencereye yeni giren birkaç maç için gerçek API çağrısı yapar) ek olarak kabaca **+300 kredi/ay** getirebilir — toplamda 500 kredi/ay limitine yakın veya bazı aylarda üzerinde kalabilir. Aşılırsa senkron sessizce başarısız olmaz, sadece o ayki eksik piyasalar için "veri yok" yazmaya devam eder (uydurma riski yok, sadece eksik kalır). Gerekirse `MAX_MATCHES_PER_RUN` düşürülebilir veya proaktif tamamlama analiz penceresiyle sınırlı tutulabilir.
